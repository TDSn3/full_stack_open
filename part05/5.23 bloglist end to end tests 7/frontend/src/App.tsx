...
function App() {
  ...
  const hook = () => {
    blogService
      .getAll()
      .then((blogsValue) => setBlogs(blogsValue.sort(
        (a: BlogType, b: BlogType) => b.likes - a.likes,
      )));
  };
  ...
  const hookBlogsChanges = () => {
    const sortedBlogsTemp = [...blogs].sort((a, b) => b.likes - a.likes);
    setSortedBlogs(sortedBlogsTemp);
  };

  useEffect(hook, []);
  useEffect(hookIsLogged, []);
  useEffect(hookBlogsChanges, [blogs]);
    ...
    <div className="App">
      {
        user === '' ? (
          ...
          <>
            <button id="logout-button" className="cta-style" onClick={handleClick} type="button">logout</button>
            <Notification message={notificationMessage} messageClassName={notificationClassName} />
            ...
            <h2>blogs</h2>
            {sortedBlogs.map((blogValue: BlogType) => (
              <ToggleBlog
                key={blogValue.id}
                buttonLabel="show"
                blog={blogValue}
